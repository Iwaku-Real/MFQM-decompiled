...
public class EntityRope extends Entity implements IProjectile, IEntityAdditionalSpawnData {
  ...
  public static Block[] quicksandIDS = new Block[] { 
      MFQM.BogBlock, MFQM.SoftSnowBlock, MFQM.SandBlock, MFQM.MorassBlock, MFQM.QuicksandBlock, MFQM.JungleQuicksandBlock, MFQM.LiquidMireBlock, MFQM.StableLiquidMireBlock, MFQM.SlimeBlock, MFQM.MireBlock, 
      MFQM.MoorBlock, MFQM.SClayBlock, MFQM.TarBlock, MFQM.LarvaeBlock, MFQM.CSandBlock, MFQM.MeatHoleBlock, MFQM.SinkingWoolBlock, MFQM.SoftQuicksandBlock, MFQM.DenseWebBlock, MFQM.SwFleshBlock, 
      MFQM.MucusBlock, MFQM.VoreHoleBlock, MFQM.MossBlock, MFQM.BrownClayBlock, MFQM.BrownClayBlock, MFQM.WetPeatBlock, MFQM.WaxBlock, MFQM.LiquidChocolateBlock, MFQM.SlurryBlock, MFQM.MudBlock, 
      MFQM.SoftGravelBlock, MFQM.HoneyBlock };
  
  public static int[] quicksandHardCoof = new int[] { 
      7, 1, 2, 20, 20, 25, 5, 5, 25, 25, 
      28, 15, 30, 5, 3, 5, 20, 25, 30, 25, 
      25, 5, 30, 15, 15, 10, 25, 25, 15, 2, 
      25, 30 };
  ...
  public EntityRope(World world, EntityPlayer par8EntityPlayer) {
    super(world);
  ...
  public EntityRope(World world, double par2, double par4, double par6) {
    super(world);
  ...
  public EntityRope(World world, double par2, double par4, double par6, EntityPlayer par8EntityPlayer) {
  ...
  public void sendPacketCurLenToServer(double cul) {
    if (this.worldObj.isRemote && 
      ...
      MFQM.network.sendToServer((IMessage)new RopeMessage((float)cul)); 
  }
  ...
  public void onUpdate() {
    ...
    if (!this.inGround) {
      ...
      if (!this.worldObj.getBlock(this.xTile, this.yTile, this.zTile).getMaterial().isSolid() && this.worldObj.getBlock(this.xTile, this.yTile, this.zTile).getMaterial() != Material.leaves)
        dis = 100.0D; 
      if (dis > 5.0D) {
        unGrabServer();
        return;
  ...
  protected void onImpact(MovingObjectPosition movingObjectPosition) {
    if (movingObjectPosition.entityHit != null) {
      if (movingObjectPosition.entityHit instanceof EntityLivingBase && 
        ...
        movingObjectPosition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, (Entity)getThrower()), 0.0F);
        this.motionX = 0.0D;
        this.motionY = 0.0D;
        this.motionZ = 0.0D;
      ...
      if (this.worldObj.getBlock(movingObjectPosition.blockX, movingObjectPosition.blockY, movingObjectPosition.blockZ).getMaterial().isSolid()) {
        this.hitted = true;
        ...
        if (checkValidToGrab(movingObjectPosition)) {
          this.inGround = true;
          this.BlockPosX = movingObjectPosition.blockX + 0.5D;
          this.BlockPosY = movingObjectPosition.blockY + 0.5D - 0.25D;
          this.BlockPosZ = movingObjectPosition.blockZ + 0.5D;
  ...
  protected boolean checkValidToGrab(MovingObjectPosition mop) {
    int xp = mop.blockX;
    int yp = mop.blockY;
    int zp = mop.blockZ;
    if (this.worldObj.getBlock(xp, yp, zp).getMaterial() == Material.leaves) {
      this.GrabType = 1;
      return true;
    } 
    if (this.worldObj.getBlock(xp, yp, zp).getMaterial().isSolid() && this.worldObj
      .getBlock(xp, yp, zp).getMaterial() != Material.sand) {
      boolean x_i = false;
      boolean x_d = false;
      boolean y_i = false;
      boolean y_d = false;
      boolean z_i = false;
      boolean z_d = false;
      if (!this.worldObj.getBlock(xp + 1, yp, zp).getMaterial().isSolid())
        x_i = true; 
      if (!this.worldObj.getBlock(xp - 1, yp, zp).getMaterial().isSolid())
        x_d = true; 
      if (!this.worldObj.getBlock(xp, yp + 1, zp).getMaterial().isSolid())
        y_i = true; 
      if (!this.worldObj.getBlock(xp, yp - 1, zp).getMaterial().isSolid())
        y_d = true; 
      if (!this.worldObj.getBlock(xp, yp, zp + 1).getMaterial().isSolid())
        z_i = true; 
      if (!this.worldObj.getBlock(xp, yp, zp - 1).getMaterial().isSolid())
        z_d = true; 
      if (x_i && x_d && z_i && z_d) {
  ...
  public void playerControl() {
    ...
    if (this.mathLen > this.brkLen) {
      if (!this.worldObj.isRemote) {
        if (ropeItem.getMetadata() < 63) {
          ...
          this.curPlayer.inventory.setInventorySlotContents(ropeNum, (ItemStack)null);
          breakSound();
        } 
    ...
    if (jumpKey() && 
      this.curLen > 1.0D)
      this.curLen = this.mathLen - Math.max(0.0D, 1.0D - Math.max(this.curPlayer.motionY * 4.0D, 0.0D)); 
    if (crouchKey() && 
      this.curLen < this.maxLen)
      this.curLen += 0.15D; 
    ItemStack curItm = this.curPlayer.getCurrentEquippedItem();
    if (curItm != null && 
      curItm.getItem() instanceof MoreFunQuicksandMod.main.items.ItemRope && 
    ...
    if (this.mathLen > this.curLen) {
      ...
      double stretch = Math.max((this.mathLen - this.curLen) / Math.min(movFac * 2.0D, 1.0D), 0.0D);
      if (!this.worldObj.isRemote) {
        if (stretch > 7.5D && 
          this.worldObj.getTotalWorldTime() % Math.max((int)Math.floor(16.0D - stretch), 1) == 0L) {
          if (this.worldObj.rand.nextInt((int)Math.floor(stretch / 10.0D) + 5) == 0)
            if (ropeItem.getMetadata() < 63) {
              ropeItem.damageItem(2, (EntityLivingBase)this.curPlayer);
              if (this.worldObj.rand.nextInt(5) == 0)
                stretchSound(); 
        ...
        double disa = Math.max((this.mathLen - this.curLen) / 10.0D, 0.0D);
  ...
  public void livingGrab() {
    ...
    if (this.mathLen > this.curLen) {
      ...
      double stretch = Math.max((this.mathLen - this.curLen) / Math.min(movFac1 * 2.0D, 1.0D) / Math.min(movFac2 * 4.0D, 1.0D), 0.0D);
      if (!this.worldObj.isRemote) {
        if (stretch > 7.5D && 
          this.worldObj.getTotalWorldTime() % Math.max((int)Math.floor(16.0D - stretch), 1) == 0L) {
          if (this.worldObj.rand.nextInt((int)Math.floor(stretch / 10.0D) + 5) == 0)
        ...
        this.curGrabbed.motionX = Math.min((this.curPlayer.posX - this.curGrabbed.posX) / this.mathLen * disa, 0.025D) * movFac2XZ + this.curGrabbed.motionX / (disa / 50.0D + 1.0D);
        this.curGrabbed.motionY = Math.min((this.curPlayer.posY + 0.5D - this.curGrabbed.posY) / this.mathLen * disa, 0.1D) * movFac2 + this.curGrabbed.motionY / (disa / 2.0D + 1.0D);
        this.curGrabbed.motionZ = Math.min((this.curPlayer.posZ - this.curGrabbed.posZ) / this.mathLen * disa, 0.025D) * movFac2XZ + this.curGrabbed.motionZ / (disa / 50.0D + 1.0D);
        ...
        if (ply != this.curPlayer && 
          ...
          double disa1 = Math.max((this.mathLen - this.curLen) / 7.5D, 0.0D);
  ...
  public int checkingInHotBar() {
    ...
    if (this.curPlayer.inventory.mainInventory[i] != null && 
      this.curPlayer.inventory.mainInventory[i].getItem() instanceof MoreFunQuicksandMod.main.items.ItemRope && (
  ...
  public int isInsideOfQuicksand(EntityLivingBase ent) {
    ...
    for (int i = 0; i < quicksandIDS.length; i++) {
      if (this.worldObj.getBlock(ppx, ppy, ppz) == quicksandIDS[i])
        return (i + 1) * 100; 
      if (this.worldObj.getBlock(ppx, ppy - 1, ppz) == quicksandIDS[i])
        return i + 1; 
    } 
    return -1;
  }
  ...
  public double getMovFactor(EntityLivingBase ent, int qs) {
    ...
    if (qs != -1)
      if (qs >= 100) {
        int trueQS = qs / 100 - 1;
        int qsHC = quicksandHardCoof[trueQS];
        moveKof /= qsHC * 0.625D;
      } else {
        int trueQS = qs - 1;
        int qsHC = quicksandHardCoof[trueQS];
    ...
    return Math.min(Math.max(moveKof, 0.01D) * 5.0D, 1.0D);
  }
  ...
  public void breakSound() {
    this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "mob.zombie.woodbreak", 0.25F, this.worldObj.rand.nextFloat() * 0.5F + 1.5F);
    this.worldObj.playSoundEffect(this.curPlayer.posX, this.curPlayer.posY, this.curPlayer.posZ, "mob.zombie.woodbreak", 0.15F, this.worldObj.rand.nextFloat() * 0.5F + 1.5F);
  }
  
  public void GrabSound() {
    this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "step.snow", 0.5F, this.worldObj.rand.nextFloat() * 0.5F + 0.5F);
  }
  
  public void unGrabSound() {
    this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "mob.bat.takeoff", 0.25F, this.worldObj.rand.nextFloat() * 0.5F + 1.5F);
    this.worldObj.playSoundEffect(this.curPlayer.posX, this.curPlayer.posY, this.curPlayer.posZ, "mob.bat.takeoff", 0.15F, this.worldObj.rand.nextFloat() * 0.5F + 1.5F);
  }
  ...
  public double getMaxYSpeed(double vel, int qs) {
    if (qs != -1) {
      ...
      if (((qs >= 3 && qs <= 5) || (qs >= 8 && qs <= 12) || (qs >= 14 && qs <= 22) || qs == 26) && 
        this.worldObj.rand.nextInt(Math.max(1000 / quicksandHardCoof[qs], 1)) != 0)
    ...
    if (!this.isRegistered && 
      ...
      for (int i = 0; i < 64; i++) {
        ...
        if (MFQM.arrayRopes[i] == null) {
          EN = true;
        } else if (!MFQM.arrayRopes[i].isEntityAlive()) {
          EN = true;
        } 
        if (EN) {
          MFQM.arrayRopes[i] = this;
          MFQM.arrayPlayers[i] = un;
