...
public class EntityVoreSlime extends EntityLiving implements IMob {
  ...
  public EntityVoreSlime(World p_i1742_1_) {
    super(p_i1742_1_);
    this.yOffset = 0.0F;
  ...
  protected EntityVoreSlime createInstance() {
    return new EntityVoreSlime(this.worldObj);
  }
  ...
  public void onUpdate() {
    ...
    if (this.onGround && !var1) {
      ...
      for (int var3 = 0; var3 < var2 * 16; var3++) {
        ...
        this.worldObj.spawnParticle(getSlimeParticle(), this.posX + var6, this.boundingBox.minY, this.posZ + var7, 0.0D, 0.0D, 0.0D);
      } 
      if (makesSoundOnLand())
    ...
    if (this.riddenByEntity == null) {
      ...
      if (!this.worldObj.isRemote) {
        ...
        ((EntityLivingBase)this.riddenByEntity).addPotionEffect(new PotionEffect(4, 100, 5, false));
        ((EntityLivingBase)this.riddenByEntity).addPotionEffect(new PotionEffect(18, 100, 5, false));
      ...
      if (this.deepFactor < 12.5F) {
        ...
        if (this.pullTime-- <= 0.0F) {
          this.pullTime = (45 + this.rand.nextInt(10));
  ...
  public void checkPlayerMuddy(EntityPlayer ent, double x, double y, double z, World world) {
    ...
    if (!world.isRemote) {
      ...
      if (preML * 0.5F > props.getMuddyLevel() * props.getMuddyTime() / 1000.0F) {
        props.setMuddyLevel(preML);
        int mdtp = 1;
        props.setMuddyType(mdtp);
        if (props.getMuddyTime() < 500)
          props.addMuddyTime(25); 
        props.setMuddyTime(Math.min(props.getMuddyTime(), 500));
      } 
    } 
  }
    ...
    if (canDamagePlayer()) {
      ...
      if (!(p_70100_1_.ridingEntity instanceof EntityVoreSlime) && !(p_70100_1_.ridingEntity instanceof EntityMuddyBlob) && !(p_70100_1_.ridingEntity instanceof EntityTarSlime) && !(p_70100_1_.ridingEntity instanceof EntitySandBlob)) {
        ...
        if (canEntityBeSeen((Entity)p_70100_1_) && getDistanceSqToEntity((Entity)p_70100_1_) < var2 * sizeCoof) {
          boolean inv = false;
          if (this.riddenByEntity == null)
            if (p_70100_1_.getHealth() < 8.0F * opMul) {
              inv = true;
          ...
          if (inv) {
            if (!isInsideOfMaterial(Material.water)) {
              float hp = p_70100_1_.getHealth() / opMul / 4.0F;
  ...
  public boolean getCanSpawnHere() {
    ...
    if (this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
      ...
      if (this.rand.nextInt((int)(1.0D + Math.floor(this.posY / 25.0D))) == 0 && this.posY < 50.0D)
