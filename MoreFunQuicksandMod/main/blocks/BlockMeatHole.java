...
public class BlockMeatHole extends Block {
  ...
  public BlockMeatHole(int mopc, int lopc, int iopc) {
    super(Material.portal);
    setBlockUnbreakable();
    setResistance(6000000.0F);
    setStepSound(Block.soundTypeGravel);
    setUnlocalizedName("TempMeat");
  ...
  public void updateTick(World world, int x, int y, int z, Random rand) {
    checkMeatNear(world, x, y, z);
  }
  
  public void checkMeatNear(World world, int x, int y, int z) {
    int xx = x;
    int yy = y;
    int zz = z;
    int met = 0;
    boolean isMeatNear = false;
    for (int re = 0; re <= 4; re++) {
      ...
      if (world.getBlock(xx, yy, zz) == MFQM.MeatBlock && 
  ...
  public void onBlockAdded(World par1World, int par2, int par3, int par4) {
    checkMeatNear(par1World, par2, par3, par4);
  }
  ...
  public void randomDisplayTick(World world, int x, int y, int z, Random random) {
    if (random.nextInt(2) == 0)
      world.playSound(x + random.nextFloat(), y + random.nextFloat(), z + random.nextFloat(), "mob.silverfish.step", 0.125F + random.nextFloat() * 0.125F, 0.75F + random.nextFloat() * 0.5F, false); 
  }
