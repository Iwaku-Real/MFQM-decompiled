...
import MoreFunQuicksandMod.main.RescueMessage;
...
public class EntityRescue extends Entity implements IProjectile, IEntityAdditionalSpawnData {
  ...
  public EntityRescue(World world) {
    super(world);
    setSize(0.25F, 0.25F);
    this.maxLen = 1.5D;
  ...
  public EntityRescue(World world, EntityPlayer par8EntityPlayer) {
    super(world);
  ...
  public EntityRescue(World world, double par2, double par4, double par6) {
    super(world);
  ...
  public EntityRescue(World world, double par2, double par4, double par6, EntityPlayer par8EntityPlayer) {
  ...
  public void sendPacketCurLenToServer(double cul) {
    if (this.worldObj.isRemote && 
      ...
      MFQM.network.sendToServer((IMessage)new RescueMessage((float)cul)); 
  }
  ...
  public void livingGrab() {
    ...
    if (this.curPlayer.isDead || !this.curPlayer.isEntityAlive())
      return; 
    int rescueNum = checkingInHotBar();
    if (rescueNum == -1)
      return; 
    EntityPlayer ply = this.curPlayer;
    if (this.worldObj.isRemote)
      ply = getPlayerMC(); 
    ItemStack rescueItem = this.curPlayer.inventory.mainInventory[rescueNum];
    if (!this.worldObj.isRemote) {
      if (this.curGrabbed == null) {
        unGrabServer();
        return;
      ...
      if (getMoCreatureValid(this.curGrabbed)) {
        this.riddenByEntity = (Entity)this.curGrabbed;
        mountEntity((Entity)this.curGrabbed);
      } 
      GrabSound();
      return;
    } 
    ...
    if (this.worldObj.isRemote && 
      (Minecraft.getMinecraft()).thePlayer == this.curGrabbed)
      minLen = 3.025D; 
    this.brkLen = this.maxLen + 0.5D;
    ...
    if (this.worldObj.isRemote && 
      this.curLen > 1.0D)
      this.curLen += (1.0D - this.curLen) / 4.0D; 
    if (jumpKey() && 
      this.curLen > 1.0D)
    ...
    if (curItm != null && 
      curItm.getItem() instanceof MoreFunQuicksandMod.main.items.ItemRescue && 
      leftKey() && 
      this.curLen > 0.75D)
      this.curLen -= 0.05D; 
    if (midKey() && 
    ...
    if (this.mathLen > this.curLen) {
      ...
      double movFac1 = getMovFactor((EntityLivingBase)this.curPlayer, qs1);
      int qs2 = -1;
      double movFac2 = 1.0D;
      if (!this.worldObj.isRemote) {
        ...
        if (this.curGrabbed != null && 
          ...
          isInsideOfQuicksand(this.curGrabbed) != -1)
          minLen = 1.25D; 
      } 
      double stretch = Math.max((this.mathLen - this.curLen) / Math.min(movFac1 * 2.0D, 1.0D) / Math.min(movFac2 * 4.0D, 1.0D), 0.0D);
      if (!this.worldObj.isRemote) {
        if (this.mathLen > minLen) {
          if (stretch > 2.5D && 
            this.worldObj.getTotalWorldTime() % Math.max((int)Math.floor(16.0D - stretch / 2.0D), 1) == 0L && 
            this.worldObj.rand.nextInt(Math.max(200 - (int)Math.floor(stretch * 10.0D), 150)) == 0) {
            unGrabServer();
            return;
          } 
          double disa = Math.max(this.mathLen - this.curLen, 0.0D);
          this.curGrabbed.motionX = Math.min((this.curPlayer.posX - this.curGrabbed.posX) / this.mathLen * disa, 0.025D) * movFac2 + this.curGrabbed.motionX / (disa / 50.0D + 1.0D);
          this.curGrabbed.motionY = Math.min((this.curPlayer.posY + 0.25D - this.curGrabbed.posY) / this.mathLen * disa, 0.1D) * movFac2 + this.curGrabbed.motionY / (disa / 2.0D + 1.0D);
          this.curGrabbed.motionZ = Math.min((this.curPlayer.posZ - this.curGrabbed.posZ) / this.mathLen * disa, 0.025D) * movFac2 + this.curGrabbed.motionZ / (disa / 50.0D + 1.0D);
          this.curGrabbed.onGround = false;
          this.curGrabbed.motionY = getMaxYSpeed(this.curGrabbed.motionY, qs2);
        } 
      } else if (this.mathLen > minLen) {
        if (ply != this.curPlayer && 
          ...
          this.curGrabbed.motionX = Math.min((this.curPlayer.posX - this.curGrabbed.posX) / this.mathLen * disa1, 0.025D) * movFac2 + this.curGrabbed.motionX / (disa1 / 50.0D + 1.0D);
          this.curGrabbed.motionY = Math.min((this.curPlayer.posY + 1.0D - this.curGrabbed.posY) / this.mathLen * disa1, 0.1D) * movFac2 + this.curGrabbed.motionY / (disa1 / 2.0D + 1.0D);
          this.curGrabbed.motionZ = Math.min((this.curPlayer.posZ - this.curGrabbed.posZ) / this.mathLen * disa1, 0.025D) * movFac2 + this.curGrabbed.motionZ / (disa1 / 50.0D + 1.0D);
          this.curGrabbed.motionY = getMaxYSpeed(this.curGrabbed.motionY, qs2);
        } 
        double disa = Math.max((this.mathLen - this.curLen) / 5.0D, 0.0D);
  ...
  public int checkingInHotBar() {
    ...
    if (this.curPlayer.inventory.mainInventory[i] != null && 
      this.curPlayer.inventory.mainInventory[i].getItem() == MFQM.RescueItem)
      return i; 
    return -1;
  }
  
  public static int isInsideOfQuicksand(EntityLivingBase ent) {
    ...
    for (int i = 0; i < EntityRope.quicksandIDS.length; i++) {
      if (ent.worldObj.getBlock(ppx, ppy, ppz) == EntityRope.quicksandIDS[i])
        return (i + 1) * 100; 
      if (ent.worldObj.getBlock(ppx, ppy - 1, ppz) == EntityRope.quicksandIDS[i])
        return i + 1; 
    } 
    return -1;
  }
  ...
  public void unGrabServer() {
    int rescueNum = checkingInHotBar();
    this.curPlayer.inventory.setInventorySlotContents(rescueNum, (ItemStack)null);
    breakSound();
    setDead();
  }
  
  public void breakSound() {
    this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "mob.wolf.step", 1.0F, this.worldObj.rand.nextFloat() * 0.25F + 1.75F);
  }
  
  public void GrabSound() {
    this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "mob.wolf.step", 1.0F, -this.worldObj.rand.nextFloat() * 0.25F + 0.5F);
  }
    ...
    if (!this.isRegistered && 
      ...
      for (int i = 0; i < 64; i++) {
        ...
        if (MFQM.arrayRescue[i] == null) {
          EN = true;
        } else if (!MFQM.arrayRescue[i].isEntityAlive()) {
          EN = true;
        } 
        if (EN) {
          MFQM.arrayRescue[i] = this;
          MFQM.arrayPlayersRescue[i] = un;
