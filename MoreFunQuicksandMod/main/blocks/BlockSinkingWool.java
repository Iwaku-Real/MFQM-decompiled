...
public class BlockSinkingWool extends BlockColored {
  public BlockSinkingWool() {
  ...
  public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
    ...
    if (entity instanceof EntityLivingBase) {
      ...
      if (!(entity instanceof EntityPlayer) && 
        MFQM.AOA && 
        world.provider.dimensionId == MFQM.AOADimCeleve)
        Affect = false; 
      if (Affect)
      ...
      if (world.getTotalWorldTime() % 128L == 0L || (entMoving && world.getTotalWorldTime() % Math.max((int)Math.floor(movCof), 1) == 0L) || (entJump && world.getTotalWorldTime() % 8L == 0L) || entSplash) {
        if (entSplash && 
          ...
          world.playSoundEffect(entity.posX, EposY, entity.posZ, "dig.cloth", 0.55F, world.rand.nextFloat() * 0.5F + 0.5F);
        } 
        if (!entSplash && entMoving && 
          ...
          world.playSoundEffect(entity.posX, EposY, entity.posZ, "step.cloth", 0.55F, world.rand.nextFloat() * 0.15F + 0.1F);
        } 
        if (entJump) {
          world.playSoundEffect(entity.posX, EposY, entity.posZ, "mob.slime.attack", 0.25F, world.rand.nextFloat() * 0.25F + 0.25F);
          world.playSoundEffect(entity.posX, EposY, entity.posZ, "dig.cloth", 0.55F, world.rand.nextFloat() * 0.15F + 0.1F);
        } 
      ...
      if (world.getBlock(x, y + 1, z) != this || (world.getBlock(x, y + 1, z) == this && world.getBlock(x, y + 2, z) == this)) {
        ...
        if (EposY - EprevPosY > 0.001D && 
          ...
          world.playSound(entity.posX, EposY, entity.posZ, "step.cloth", 0.35F, world.rand.nextFloat() * 0.15F + 0.1F, false);
        } 
        if (entJump && 
  ...
  public void updateTick(World world, int x, int y, int z, Random rand) {
    if (world.rand.nextInt(5) == 0) {
      if (!world.getBlock(x, y - 1, z).getMaterial().isSolid() && world.getBlock(x, y - 1, z).getMaterial().isReplaceable()) {
        world.setBlock(x, y - 1, z, (Block)this, world.getBlockMetadata(x, y, z), 3);
        world.setBlockToAir(x, y, z);
      } 
    } else {
      world.scheduleBlockUpdate(x, y, z, (Block)this, tickRate(world) * 10);
    } 
  }
  
  public void onBlockAdded(World par1World, int par2, int par3, int par4) {
    par1World.playSoundEffect((par2 + 0.5F), (par3 + 0.5F), (par4 + 0.5F), "mob.slime.attack", 0.45F, par1World.rand.nextFloat() * 0.5F + 0.5F);
