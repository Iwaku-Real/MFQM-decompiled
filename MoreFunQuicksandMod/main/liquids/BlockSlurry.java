...
public class BlockSlurry extends BlockFluidClassic {
  public IIcon[] theSlurryIIcon;
  
  public static final Material materialSlurry = (new CustomMaterialLiquid(MapColor.dirtColor)).setReplaceable();
  ...
  public BlockSlurry(int mopc, int lopc, int iopc) {
    super(MFQM.SlurryFluid, materialSlurry);
  ...
  public boolean CheckPoisonProtect(Entity entity) {
    if (!(entity instanceof EntityPlayer))
      return false; 
    if (((EntityPlayer)entity).getCurrentArmor(0) == null)
      return false; 
    if (((EntityPlayer)entity).getCurrentArmor(1) == null)
      return false; 
    if (((EntityPlayer)entity).getCurrentArmor(2) == null)
      return false; 
    if (((EntityPlayer)entity).getCurrentArmor(3) == null)
      return false; 
    String str1 = ((EntityPlayer)entity).getCurrentArmor(1).getItem().getUnlocalizedName();
    String str2 = ((EntityPlayer)entity).getCurrentArmor(2).getItem().getUnlocalizedName();
    String str3 = ((EntityPlayer)entity).getCurrentArmor(3).getItem().getUnlocalizedName();
    int pProtection = 0;
    if (str1.matches("(.*)Hazmat(.*)")) {
      pProtection++;
    } else if (str1.matches("(.*)Nano(.*)")) {
      pProtection++;
    } else if (str1.matches("(.*)Quantum(.*)")) {
      pProtection++;
    } else if (str1.toLowerCase().matches("(.*)poison(.*)")) {
      pProtection++;
    } else {
      return false;
    } 
    if (str2.matches("(.*)Hazmat(.*)")) {
      pProtection++;
    } else if (str2.matches("(.*)Nano(.*)")) {
      pProtection++;
    } else if (str2.matches("(.*)Quantum(.*)")) {
      pProtection++;
    } else if (str1.toLowerCase().matches("(.*)poison(.*)")) {
      pProtection++;
    } else {
    ...
    if (str3.matches("(.*)Hazmat(.*)")) {
      pProtection++;
    } else if (str3.matches("(.*)Nano(.*)")) {
      pProtection++;
    } else if (str3.matches("(.*)Quantum(.*)")) {
      pProtection++;
    } else if (str1.toLowerCase().matches("(.*)poison(.*)")) {
      pProtection++;
    } else {
  ...
  public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
    ...
    if (entity instanceof EntityLivingBase) {
      ...
      int mr_kof = (int)Math.min(5.0D + Math.floor(Math.max(0.0D, Math.pow((mtdkof * 5.0F) * (1.5D - kof1m), 1.5D))), 50.0D);
      ...
      if (world.getBlock(x, y + 1, z) != this || (world.getBlock(x, y + 1, z) == this && world.getBlock(x, y + 2, z) == this)) {
        ...
        if (!world.isRemote) {
          ...
          oy3 *= -1.0D;
          double kof3 = oy3;
          boolean PoisonProtect = false;
          boolean PoisonProtectA = false;
          if (kof3 < 0.5D) {
            if (world.getTotalWorldTime() % Math.floor(Math.max(kof3 * 1000.0D, 1.0D)) == 0.0D) {
              if (!PoisonProtectA) {
                PoisonProtect = (PoisonProtect || CheckPoisonProtect(entity));
                PoisonProtectA = true;
              } 
              if (!PoisonProtect) {
                ((EntityLivingBase)entity).addPotionEffect(new PotionEffect(9, 200, 0, false));
                ((EntityLivingBase)entity).addPotionEffect(new PotionEffect(17, 1200, 0, false));
              } 
            } 
            if (world.getTotalWorldTime() % Math.floor(Math.max(kof3 * 3000.0D, 1.0D)) == 0.0D) {
              if (!PoisonProtectA) {
                PoisonProtect = (PoisonProtect || CheckPoisonProtect(entity));
                PoisonProtectA = true;
              } 
            ...
            if (world.getTotalWorldTime() % Math.floor(Math.max(kof3 * 5000.0D, 1.0D)) == 0.0D) {
              if (!PoisonProtectA) {
                PoisonProtect = (PoisonProtect || CheckPoisonProtect(entity));
                PoisonProtectA = true;
              } 
              if (!PoisonProtect)
                ((EntityLivingBase)entity).addPotionEffect(new PotionEffect(20, 600, 1, false)); 
            } 
          } 
        } 
        double mys = 0.0D;
  ...
  public IIcon getIcon(int par1, int par2) {
    return (par1 != 0 && par1 != 1) ? this.theSlurryIIcon[1] : this.theSlurryIIcon[0];
  }
  ...
  public int checkFusion(World world, int x, int y, int z, Random rand) {
    ...
    if (isLavaNear) {
      world.setBlock(x, y, z, MFQM.MudBlock, 0, 3);
  ...
  public void updateTick(World world, int x, int y, int z, Random rand) {
    ...
    if (world.getBlockMetadata(x, y, z) == 0) {
      if (world.getBlock(x, y + 1, z) == MFQM.MeatBlock && 
        world.getBlockMetadata(x, y + 1, z) == 10)
    ...
    if (!world.isRemote && 
      ...
      world.getBlock(x, y + 1, z) == Blocks.air || world.getBlock(x, y + 1, z) == MFQM.GasBlock)) {
      world.setBlock(x, y + 1, z, MFQM.GasBlock, 15, 2);
      world.playSoundEffect(x + rand.nextFloat(), y + 0.5D, z + rand.nextFloat(), "liquid.lava", 1.0F, rand.nextFloat() * 0.5F + 1.0F);
      world.spawnEntityInWorld((Entity)new EntityLongStick(world, x + 0.5D, (y + 1), z + 0.5D, null, x, y, z));
    } 
    this.quantaPerBlockFloat = 1.75F;
  ...
  public void registerIcons(IIconRegister par1IIconRegister) {
    this.theSlurryIIcon = new IIcon[] { par1IIconRegister.registerIcon("morefunquicksandmod:Slurry_Still"), par1IIconRegister.registerIcon("morefunquicksandmod:Slurry_Flowing") };
  }
